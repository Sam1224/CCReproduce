...
class SimpleTokenizer:
    ...
    def encode_batch(self, texts: List[str], max_len: int = 32) -> TokenizedBatch:
        ids: List[List[int]] = []
        masks: List[List[int]] = []
        for t in texts:
...
class DualEncoder(nn.Module):
    """Siamese dual encoder (shared weights), like the paper's Qwen3-Embedding backbone."""
    ...
    def forward(
        self,
        query_batch: TokenizedBatch,
        doc_batch: TokenizedBatch,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        q = self.encode(query_batch.token_ids, query_batch.attn_mask)
        d = self.encode(doc_batch.token_ids, doc_batch.attn_mask)
        return q, d
